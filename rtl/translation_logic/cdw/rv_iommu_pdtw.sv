...
// Copyright © 2026 (PR-cdw-split v4.3: revert v4.2 BURST_FIXED → BURST_INCR)
// SPDX-License-Identifier: Apache-2.0 WITH SHL-2.1
//
// Description: Process Directory Table Walker (PDTW), v4.3
//
//   v4.3 changes (vs v4.2):
//     - 全 AR を BURST_INCR に戻す (= v4.2 の FIXED + split は誤り)
//     - LEAF_S1 を multi-beat (len=1, 2 beats) read に戻す
//     - DC / PC は 8 byte より大きいので multi-beat read が必要
//     - axi4_bc が BURST_INCR + len=0 を hang する説は誤り
//       (= 旧 PTW hang の原因は別、 たまたま FIXED にしたら直っただけ)
...
#(
    ...
    always_comb begin : pdtw_fsm
        ...
        // ── AR デフォルト: 元の v4 と同じ (= BURST_INCR、 LEAF は multi-beat) ──
        mem_req_o          = '0;
        mem_req_o.ar.id    = 4'b0001;
        mem_req_o.ar.addr  = {{riscv::XLEN-riscv::PLEN{1'b0}}, cdw_pptr_q};
        mem_req_o.ar.len   = (phase_q == PH_LEAF_S1) ? 8'd1 : 8'd0;   // LEAF=2-beat, NL=1-beat
        mem_req_o.ar.size  = 3'b011;
        mem_req_o.ar.burst = axi_pkg::BURST_INCR;                      // ★ INCR (= multi-beat 対応)
        ...
        case (state_q)
            ...
            ST_WAIT: begin
                case (phase_q)
                    ...
                    PH_NL_S2, PH_LEAF_S2: begin
                        // S2 walk は AXI を通らない (= ptw_done_i 待ち) のでそのまま
                        if (ptw_done_i) begin
            ...
            // ── multi-beat read 中の error は r.last 待ちで drain
            ST_ERROR: begin
