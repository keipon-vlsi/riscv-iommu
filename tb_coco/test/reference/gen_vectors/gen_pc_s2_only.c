// gen_pc_s2_only.c — PDTV=1 + PC.fsc=Bare + S2=Sv39x4 の PTE フラグ網羅
//
// =============================================================================
//  gen_s2_only.c の PC (Process Context) 版。
//
//  固定軸
//   - DC.tc.PDTV = 1、DC.fsc.pdtp.MODE = PD20
//   - process_id = PC_PROCESS_ID_FIXED (0x42)
//   - PC.ta.PSCID = PC_PSCID_FIXED (0x100)、V=ENS=SUM=1
//   - PC.fsc.iosatp.MODE = Bare (S1 = identity)
//   - DC.iohgatp = Sv39x4 (S2 active)
...
int main(int argc, char *argv[]) {
    ...
    for (int flags = 0; flags < 256; flags++) {
        for (int acc = 0; acc < 2; acc++) {
            ...
            snprintf(tc.name, sizeof tc.name,
                     "pc_s2_leaf_lvl0_f%02x_%s", flags, acc == 0 ? "r" : "w");
            strncpy(tc.category, CATEGORY, sizeof tc.category - 1);
            tc.stage_mode   = STAGE_PC_S2_ONLY;
            tc.level        = 0;
    ...
    for (int level = 1; level <= 2; level++) {
        for (int flags = 0; flags < 256; flags++) {
            ...
            tc.case_id = case_id++;
            snprintf(tc.name, sizeof tc.name,
                     "pc_s2_nonleaf_lvl%d_f%02x", level, flags);
            strncpy(tc.category, CATEGORY, sizeof tc.category - 1);
            tc.stage_mode   = STAGE_PC_S2_ONLY;
            tc.level        = level;
            tc.flags        = (uint8_t)flags;
    ...
    for (int b = 0; b < 9; b++) {
        test_case_t tc = {0};
        tc.case_id = case_id++;
        snprintf(tc.name, sizeof tc.name, "pc_s2_rsvd_single_bit%d", b + 54);
        strncpy(tc.category, CATEGORY, sizeof tc.category - 1);
        tc.stage_mode   = STAGE_PC_S2_ONLY;
    ...
    for (int i = 0; i < 90; i++) {
        ...
        test_case_t tc = {0};
        tc.case_id = case_id++;
        snprintf(tc.name, sizeof tc.name, "pc_s2_rsvd_random_%03d", i);
        strncpy(tc.category, CATEGORY, sizeof tc.category - 1);
        tc.stage_mode   = STAGE_PC_S2_ONLY;
        tc.level = 0; tc.flags = 0; tc.access = ACC_READ;
        tc.rsvd_pattern = p;
