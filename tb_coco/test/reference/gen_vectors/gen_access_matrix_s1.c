// gen_access_matrix_s1.c — S1 PTE パーミッション × アクセス種別 直積 (STAGE_S1_ONLY)
//
...
#define CATEGORY      "access_matrix_s1"
...
int main(int argc, char *argv[]) {
    ...
    for (int x = 0; x < 2; x++) {
        ...
        for (int ai = 0; ai < 2; ai++) {
            test_case_t tc = {0};
            tc.case_id = case_id++;
            snprintf(tc.name, sizeof tc.name, "s1_u%d_r%d_w%d_x%d_acc%s", u, r, w, x, AS[ai]);
