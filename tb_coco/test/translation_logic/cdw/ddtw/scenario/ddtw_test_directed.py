...
配置: /workspace/tb_coco/test/translation_logic/cdw/ddtw/scenario/ddtw_test_directed.py
...
async def test_d02_1lvl_ddt_with_s2(dut):
    ...
    g_root   = pmm.alloc_aligned_ppn(4)         # ★ 16K aligned (= ppn[1:0]=0) を取る
    pdt_gppn = pmm.alloc_ppn()                  # g_root+1
    pdt_spa  = pmm.alloc_ppn()                  # g_root+2
    did      = 0x10

    # 16K aligned (= ppn[1:0]=0) を sanity check
